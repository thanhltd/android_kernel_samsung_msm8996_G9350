...
static void max778xx_haptic_en(struct ss_vib *vib)
{
	...
	case CHIP_MAX77833:
		max77833_vibtonz_en(!!atomic_read(&vib->cnt));
		break;
#endif
#if defined(CONFIG_MOTOR_DRV_MAX77854)
	case CHIP_MAX77854:
		max77854_vibtonz_en(!!atomic_read(&vib->cnt));
		break;
#endif
	default:
		break;
...
static void set_vibrator(struct ss_vib *vib)
{
	...
	pr_info("[VIB]: %s, value[%d]\n", __func__, !!atomic_read(&vib->cnt));
	if (!!atomic_read(&vib->cnt)) {
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	mutex_lock(&vib->lock);
	ret = hrtimer_cancel(&vib->vib_timer);
	if (ret)
		atomic_dec(&vib->cnt);
